...
interface Props {
    ...
    titleStyle?: React.CSSProperties; // New prop for title style
}

export const ClickableList = ({ items, title, onChoseItem, titleStyle }: Props) => {
    const [searchedValue, setSearchedValue] = useState("");
    return <>
        <Typography variant='h4' align='center' sx={titleStyle}>{title}</Typography>
        <Autocomplete
            freeSolo
            options={items}
